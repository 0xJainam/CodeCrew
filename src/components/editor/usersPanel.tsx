    ...
    <div className="w-56 border-l border-border-default bg-surface-50/60 backdrop-blur-xl flex flex-col">
      ...
      <div className="flex-1 px-2 pb-4 space-y-0.5">
        {users.map((user) => (
          ...
          >
            {/* Avatar initial */}
            <div className={`w-6 h-6 rounded-md flex items-center justify-center text-[10px] font-bold ${getAvatarStyle(user)}`}>
              {user.charAt(0).toUpperCase()}
            </div>
            <div className="flex items-center gap-2 min-w-0">
              <span className="text-[13px] font-medium truncate">{user}</span>
              {user === "You" && (
